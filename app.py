...
app.config.update(
    ...
    CACHING=True,
    CACHE_TYPE='SimpleCache'  # Use string identifier with capital 'S'
)
...
def fetch_arbitrage_opportunities():
    ...
    try:
        ...
        try:
            ...
            return jsonify(error='Investment, slippage and fee must be numbers.'), 400
        if not all(math.isfinite(v) for v in (investment_amount, slippage_rate, transaction_fee)) or investment_amount <= 0 or not 0 <= slippage_rate < 1 or not 0 <= transaction_fee < 1:
            return jsonify(error='Investment must be positive; slippage and fee must be fractions from 0 to less than 1.'), 400
        contract_address = request.form.get('search', '').strip() or '7vfCXTUXx5WJV5JADk17DUJ4ksgau7utNKj4b963voxs'
        ...
        arbitrage_results = process_arbitrage_data(
            ...
            include_unprofitable=request.form.get('include_unprofitable') == 'true'
        )
